...
/**
 * ------------------------------------------------------------------
 * 1. CONFIGURATION (SECURE ENV VARS)
 * ------------------------------------------------------------------
 */

const USE_FIREBASE = true; 

// ACCESSING ENV VARS DIRECTLY SO VITE CAN SEE THEM
...
if (USE_FIREBASE) {
  try {
    // Only initialize if the key exists (prevents crashing if env vars are missing)
    if (FIREBASE_CONFIG.apiKey) {
      ...
      console.warn("⚠️ Firebase keys missing from Environment Variables.");
    }
  } catch (error) {
